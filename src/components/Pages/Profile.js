import React from 'react'
import Student, { myName } from "./Data"
// import {Ali,name1, Name2, Name3} from './Data'
// oper waly import ko as be used kr skty ha somes ak object ha us me data store kia ha
import * as somes  from './Data'

// const Login =()=>{
//   return <h1>Login</h1>
// }
// const Dashboard =()=>{
//   return <h1>Dashboard</h1>

// }


export  function Profile({userfirstName,userlastName, userfulName}) {
  // is sy koe b conditon tue nhi ho gi 
  // const isUserLoginIn = "null";

  //is sy dahbord show ho ga
// const isUserLoginIn = "false";

//login show ho ga
const isUserLoginIn = "true";
// if(!isUserLoginIn)
// {
//   return Dashboard()
// }
// if(isUserLoginIn)
// {
//   return Login()
// }


  console.log(userfirstName,userlastName,userfulName)
const aler =()=>{
  alert("Saqib ali")
}

//button k lia function
const handleClick =() =>{
  console.log("Click Button");
  alert("Saqib ali")
}
// /button function two
const handClickTwo =(parm1 ,event,buttonpurpose)=>{
console.log(parm1)
alert(parm1);
console.log(event.type);
console.log(event.timeStamp);
console.log(buttonpurpose);
let now = new Date();
console.log(now.getHours(), ":" ,now.getMinutes());

}

//edits paragraph
const edits = "Type Your Paragraph"

//images k lia
let img1 = "data:image/png;base64,";
let img2 = "https://i.ytimg.com/vi/1iphT2E2jZY/sddefault.jpg"; 
let link= "http://localhost:3000/"


const openInNewTab = url =>{window.open(url, '_blank', 'noopener,noreferrer');}

//Spread Operator in ReactJS
const arr1 = ["Saqib" , "Ali", "Bscs", 34]; 
const arr2 = ["Tamoor" , "Ali", "Bscs", 44]; 

var combine = [...arr1, ...arr2];

//method 2
var arr3 = ["Saqib Ali", ":", "Bilal: "]
var arr5 = ["FSD :","  Faisalabad"]
var arr4 = ["Name:", ...arr3 , "City :", ...arr5] ;



//method 3 
var stu = {
  Name : "SAQIB",
  LName: "Ali"

};

const data = {
  id: 1,
  ...stu,
  age: 26,
  Gender: "Male"
};

// console.log(data)













return (

    

    <div className='profile'>

<h1> JSX Attribute in React</h1>
<h3>Edit Peragraph ::::: contentEditable = true</h3>
<textarea contentEditable ="true" placeholder='Type Your Paragraph '>{edits}</textarea>

    <h1>Profile</h1>

    <p>User First Name is = {userfirstName}</p>
    <p>User Last Name is = {userlastName}</p>
    <p>User Name is = {userfulName}</p>
    <p onClick={ aler}>User Name is = {userfulName} </p>


    {/* <button onClick={()=>{console.log('i m running')}}>Click me</button> */}
    
    <button onClick={handleClick}>Click me Alert</button>
    {/* <button onClick={()=>{handClickTwo("Parameter Pass One" ,"Parameter Pass Two" ) }}>Parameter Pass</button> */}
    <button onClick={(event)=>{handClickTwo("Parameter Pass One"  ,event ,"Whatts App") }}>Parameter Pass</button>
<br/>
<br/>
<h1>image tag used react</h1>
<img src ={img1} target="_blank"></img><br/>
<p onClick={()=>openInNewTab('https://google.com')}>New Window Google.comm star</p>
<img src ={img1}></img>
<img src ={img1}></img>
<img src ={img1}></img>

<h1>Link Used in React JS</h1>
<a href={link} target="_blank">
 <img src ={img2}   alt='Image'/>
 <br/>
 <b>Click Me</b>
 </a>

<hr/>
<hr/>
<h1>ES6 Modules Import Export in React JS</h1>
{/* <p>{Ali}</p>
<ol>
  <li>{name1}</li>
  <li>{Name2}</li>
  <li>{Name3}</li>
</ol> */}

<p>{somes.Ali}</p>
<ol>
  <li>{somes.name1}</li>
  <li>{somes.Name2}</li>
  <li>{somes.Name3}</li>
</ol>

 <p>{Student}</p>
 <hr/>
 <hr/>

 <h1> Spread Operator in ReactJS</h1>
 <h2>Method 1</h2>
<p>{combine}</p>
<h2>Method 2</h2>
<p>{arr4}</p>
<h2>Method 3</h2>
<span>console.log </span>








    </div>
  )
}
